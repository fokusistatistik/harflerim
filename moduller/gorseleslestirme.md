...
2. ⚠️ **Siluet CSS ile üretilmeye çalışıldı — bu turdaki deneme (`grayscale(1) brightness(0.4) contrast(3)`) yetersiz kaldı.** Koyu renkli nesnelerde (ör. siyah köpek) filtre arka planla birleşip yarı saydam, "x-ray" gibi düz bir gri blok üretti — kullanıcı bunu görünce "silüet hiç belli olmuyor" diye işaretledi. Kesin çözüm 4. turda bulundu, bkz. aşağıdaki bölüm.
...
5. ⚠️ **Progress göstergesi ("Bugün N/Limit") eklendi ama konumlandırma tam düzelmedi.** İlk denemede `GameHud`'ın `center` prop'u + `absolute` header içinde metin, alttaki siluet kutusunun İÇİNE düşüyordu. `pt-*` padding artırma denemeleri (flex `justify-center` içeriği aşağı itmek yerine kullanılabilir alanı daraltıyor) işe yaramadı — kullanıcı ekran görüntüsüyle "hem üstte gereksiz boşluk var hem de progress metni hâlâ yanlış yerde" bulgusunu iletti. Kesin çözüm 4. turda bulundu.
...
## Dördüncü tur (2026-09-15) — siluet kontrastı ve boşluk sorunu kesin olarak çözüldü

Kullanıcı 3. turun sonucunu (yarı saydam gri blok, üstte boşluk) görünce net bir geri bildirim verdi: *"şuan böyle hem üstte gereksiz boşluk var hem de xray gibi duruyor"*. İki kök neden bulunup düzeltildi:

1. ✅ **Siluet: SVG `<filter>` ile eşikli (thresholded) düz renkli siluete geçildi.** CSS `filter` kombinasyonları (grayscale/brightness/contrast) ve `mask-image`/`mask-mode: luminance` (JPEG'lerde tarayıcı bazında güvenilmez çalıştı, görsel hiç maskelenmedi) ikisi de nesnenin kendi renginden bağımsız değildi. Kesin çözüm: `feColorMatrix` ile gri tonlamaya çevrilip `feComponentTransfer`/`feFuncA` ile luminance %70 eşiğinde ikili (binary) bir alfa maskesi üretiliyor, `feFlood` + `feComposite` ile bu maskenin üzerine düz bir `papatya-ink-soft` rengi dolduruluyor — sonuç, nesnenin rengi ne olursa olsun tutarlı, keskin, tek renkli gerçek bir siluet. Farklı round'larda (sırt çantası, deve, lale, balık kavanozu, valiz) gerçek tarayıcıda görsel olarak doğrulandı — yalnızca çok gürültülü dokulu nesnelerde (ör. üstü serpme donut) siluet biraz dağınık çıkıyor, kabul edilebilir bir sınır.
2. ✅ **Layout: Harf Avı'nın kanıtlanmış deseni birebir uygulandı.** `GameHud` artık `absolute` değil — Harf Avı'ndaki gibi normal akışta (`w-full p-4 lg:p-8 pb-0 shrink-0`), progress göstergesi `center` prop'unda header'ın kendi satırında (Harf Avı'daki "Tur N" rozetiyle aynı görsel dil — `bg-papatya-petal/15` yuvarlak rozet). Altındaki içerik `flex-1` ile geri kalan tüm alanı doğal olarak dolduruyor. Bu, hem "üstte gereksiz boşluk" hissini giderdi hem de önceki iki turda çözülemeyen progress-metni-siluetin-içine-biniyor bug'ını kesin olarak ortadan kaldırdı (kök neden: `absolute` header + `pt-*` padding kombinasyonu güvenilir değildi, normal flex akışı güvenilir).

**Doğrulama (4. tur):** `tsc --noEmit` ve `eslint` temiz. Gerçek tarayıcıda 375px/800px/1920px'te: 5+ farklı round'da siluet görsel olarak net/tutarlı, progress göstergesi header altında kendi rozetinde (hiçbir örtüşme yok), doğru kart sürüklemesi (fare VE klavye) başarılı eşleşmeye yol açıyor, masaüstünde yan yana/mobilde dikey düzen korunuyor, konsol hatası sıfır, scroll taşması yok.
...
- Çok gürültülü dokulu nesnelerde (ör. serpme kaplı donut) siluetin dağınık çıkması — eşik ince ayarı ya da bu tür fotoğrafların havuzdan hariç tutulması değerlendirilebilir, küçük bir kusur.
- Blok (a)-(g) metinlerinin (dosyanın üst kısmı) 3-4. turdaki harf→nesne geçişini yansıtacak şekilde tam olarak yeniden yazılması — şu an yalnızca bu son iki özet güncel, üstteki bloklar hâlâ harf-tabanlı mekaniği anlatıyor (sonraki bir turda ele alınabilir).
