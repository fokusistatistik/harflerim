...
**Sürüm 2.12 · 13 Eylül 2026 — Mühürlenmiş**
...
| 1.19 | **Aile/Guardian veri modeli (yalnızca şema)** *(3. loop'ta keşfedilen boşluk)* | Şu an "ebeveyn" hiçbir veri varlığı değil — yalnızca çocuğun `UserSettings`'inde bir PIN. Yönetişim bölümündeki "Aile yapısı" ilkesi ("`User` ile ebeveyn kimliği asla birebir eşlenmemelidir") ve Faz 3.12'nin "tek bir ebeveyn PIN'i tüm çocuk profillerini yönetir" gerekliliği için yeni bir `Guardian` tablosu + `User.guardianId` (nullable) eklenir. **Bilinçli olarak yalnızca şema** — mevcut `parentPin`/`ParentGate` akışına dokunulmaz, kullanılmaya Faz 3.12'de başlanır. 1.4b ile aynı desen. | Orta |
| 1.20 | **Beceri/kazanım (Skill) katmanı** *(3. loop'ta keşfedilen boşluk, Faz 3.1'in önkoşulunun erken temeli)* | Oyun-bağımsız `Skill` + `SkillAttempt` tabloları ve genel bir `recordSkillAttempt()` yardımcı fonksiyonu eklenir. **Dört oyunun tamamı** (Harf Avı, Hafıza Kartları, Sihirli Kelimeler, Görsel Eşleştirme) kendi denemelerini bu ortak katmana da yazacak şekilde güncellenir — GENEL-KURALLAR md.3'teki "veri temelli yaklaşım" ilkesinin ve Faz 3.1/3.2/3.8/2.8b'nin dayanacağı ölçüm temeli erkenden atılmış olur. Zorluk ayarlama mantığının kendisi hâlâ Faz 3.1'in işi — burada yalnızca ölçüm/kayıt var. | Yüksek |
| 1.21 | **Dil/metin katmanını tekilleştir** *(3. loop'ta keşfedilen boşluk — Faz 4.3 Çoklu dil'in önkoşulu)* | `src/locales/tr.json` yazılmış ama tamamen kullanılmıyor (hâlâ eski "HarfArkadaşım" başlığıyla) — gerçek Türkçe metinler (`useTurkishSpeech.ts`'teki tebrik/teşvik cümleleri dahil) bileşenlere dağınık ve bu dosyayla kısmen çakışıyor/tekrarlıyor. Tüm kullanıcıya görünen Türkçe metin tek bu dosyaya taşınır, ince bir `t()` yardımcısı üzerinden okunur. Çoklu dile geçiş (Faz 4.3) artık gerçek bir temelden ilerler. | Orta |
| 1.22 | **Oyun ilerlemesini tam veritabanına taşı** *(3. loop'ta keşfedilen boşluk)* | Hafıza Kartları ve Sihirli Kelimeler ilerlemesi hâlâ `localStorage`'da — Faz Çıkışı'nın "ilerlemesi hesabına kaydedilir" vaadiyle çelişiyor, cihaz değişince/tarayıcı verisi silinince sıfırlanıyor, Faz 2.1 ebeveyn paneli bu veriyi hiç göremeyecek. Harf Avı'nın zaten kullandığı `Session` tablosuna (kendi `gameId`'leriyle) taşınır. | Yüksek |
| 1.23 | **Kişiselleştirme: hardcoded "Melike" referansları** *(3. loop'ta keşfedilen boşluk — 1.9'un asıl istediği ama atlanan iş)* | `page.tsx`, `GameBoard.tsx` ve `MemoryMatchGame.tsx`'te sabit "Melike ile Harfleri Keşfet" metni ve avatar görseli var — başka bir çocuk hesabıyla girilse bile değişmiyor. Metin `firstName`/`worldName()`'a bağlanır. (Avatar görselinin kendisi ayrı bir profil-fotoğrafı özelliği gerektirir — bu, Faz 2.2 "Aile bireyleri kaydı"nın işi, kapsam dışı.) | Düşük |
| 1.24 | **ParentFooter'ı oyun-bağımsız hale getir** *(3. loop'ta keşfedilen boşluk)* | "Seviye X/24" ve "Hedef: X/24" hâlâ yalnızca Harf Avı'nı yansıtıyor; diğer 3 oyun oynanırken de aynı alakasız veriyi gösteriyor. Genel (oyun-bağımsız) bir özete — ör. günlük süre bütçesi ilerlemesi — dönüştürülür. | Düşük |
| 1.25 | **Ekran sağlığı/güvenlik-kritik kod için test kapsamını genişlet** *(3. loop'ta keşfedilen boşluk)* | `src/lib/dailyUsage.ts` (`addDailyUsageSeconds`/`getDailyUsage`) için özel birim testler yok — yalnızca dolaylı olarak test ediliyor. Ekran sağlığı güvenlik-kritik olduğu için bu boşluk kapatılır. | Orta |
| 1.26 | **Küçük temizlik** *(3. loop'ta keşfedilen boşluk)* | `README.md` hâlâ eski "HarfArkadaşım"/localStorage mimarisini anlatıyor — Papatya'ya güncellenir. `games/magic-words` ve `games/memory-match` sayfalarındaki zararsız Next.js `viewport` metadata uyarısı (ayrı `export const viewport` gerekiyor) düzeltilir. | Düşük |
...
**1.19-1.26 not** — Bu maddeler 3. loop'un genel denetiminde bulundu: bir kısmı sonraki fazların (3.1, 3.12, 4.3) hiç başlamamış altyapı temeli, bir kısmı mevcut işteki tutarsızlık/eksiklik düzeltmesi. Faz 1'in "sağlam temel" ilkesini tamamlıyorlar, yeni bir ürün özelliği eklemiyorlar.

**Uzman denetimi** — Faz kapanmadan önce arayüz sadeliği, giriş ritüelinin bilişsel yükü ve süre limitinin uygulanış biçimi çocuk gelişimi bakış açısıyla gözden geçirilir. *(Bu, koddan bağımsız bir adımdır — henüz yapılmadı, kullanıcının kendisinin organize etmesi gerekir.)*
