...
**Sürüm 2.8 · 12 Eylül 2026 — Mühürlenmiş**
...
| 1.17 | **Duyusal ayarların merkezi bağlanması** *(1. loop'ta keşfedilen boşluk)* | `UserSettings.dailyScreenLimit`, `reduceMotion`, `highContrast`, `speechEnabled` şemada var ama hiçbir yere bağlı değil: `game.ts`'deki `GAME_LIMITS` hâlâ oyun başına sabit bir sayı (kullanıcıya özel değil), token katmanı yalnızca tarayıcının `prefers-reduced-motion`'ını okuyor (kullanıcının kendi tercihini değil), `AudioProvider` `speechEnabled=false` olsa bile konuşmayı dener. **1.10 (Ekran sağlığı) ve 1.9 (görsel kimlik) bu bağlantı kurulmadan tam çalışamaz** — bu yüzden onlardan önce ele alınmalı. | Yüksek |
| 1.18 | **Navigasyon iskeleti ve çıkış kontrolü** *(2. loop'ta keşfedilen boşluk)* | Şu an çıkış (logout) yapacak hiçbir arayüz kontrolü yok — `logout()` action'ı yazılı ama hiçbir düğmeye bağlı değil; `ParentFooter`'daki kilit ikonu tamamen dekoratif. Bu maddede: **(1)** Çıkış kontrolü, çocuğun yanlışlıkla basamayacağı şekilde 1.3'teki ebeveyn kapısının içine yerleştirilir — çocuk yüzeyine serbestçe konmaz. **(2)** Kalıcı bir navigasyon iskeleti kurulur: Faz 1-3'te planlanan **tüm alanlar** (3 oyun, müzik köşesi, çizgi film, aile albümü, çizim tahtası, yazı alıştırması, AAC iletişim tahtası, ebeveyn yönetim alanı) en azından teorik bir yuva/rota olarak menüde yer alır — henüz yazılmamış olanlar "yakında" durumunda, sakin ve kilitli görünür, tamamen görünmez değildir. **(3)** `Header`/`ParentFooter` 1.6'daki token katmanına taşınır — bu, navigasyon iskelesi için temel tasarım standardını oluşturur. | Yüksek |

**Faz Çıkışı** — Melike kendi adıyla giriş yapar, *Melike'nin Dünyası* onu karşılar, uygulama onunla Türkçe konuşur, üç oyun da telefonda ve PC'de düzgün ölçeklenir, ilerlemesi hesabına kaydedilir, günlük süre limiti fiilen çalışır ve internet olmadan da her şey açılır. Yeni bir kelime eklemek artık kod değişikliği gerektirmez. Ebeveyn, çocuğun ilgi alanlarını ve duyusal profilini kaydedebilir — bu veri henüz kullanılmasa da (Faz 2.10'u bekler) şemada hazır durur. Her ebeveyn işlemi audit log'a düşer, bildirimler tek merkezi kaynaktan çıkar, hesap silme talep edilirse zorunlu yedek + vazgeçme penceresiyle güvence altına alınır, çocuk uygulamayı arka plana attığında ebeveyn haberdar olur. Ebeveyn kapısından çıkış yapılabilir, menüde Faz 3'e kadar planlanan tüm alanlar — henüz yazılmamış olsa bile — en azından bir yuva olarak görünür.
